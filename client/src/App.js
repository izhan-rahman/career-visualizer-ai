import React, { useState, useRef, useEffect, useCallback } from "react";
    ...
    // --- This URL now points to your LIVE PythonAnywhere server ---
    const SERVER_URL = 'https://izhanrahman.pythonanywhere.com';
    // --------------------------------------------------
    ...
    function CareerVisualizerApp({ onLogout, role }) {
      ...
      // This function now includes your fallback logic
      const getCareerData = useCallback((career) => {
        ...
        if (imagePath) {
          ...
          return {
            ...
            src: '/images/confused.jpeg', // Make sure you have this image
            isUnknown: true
          };
        }
      ...
      const processVoiceCommand = useCallback((transcript) => {
        ...
        setName(dName); 
        if (dCareer) {
          const careerData = getCareerData(dCareer); // Get title, image, and status
          setCareerImage(careerData);
          setStage("result");
          
          // Only save if the career is known
          if (!careerData.isUnknown) {
